...
export const statusLabels: Record<string, string> = {
  ...
  researching: "Ищем контакт",
  ready: "Готов к контакту",
  contacted: "Связались",
  replied: "Ответил",
  ...
  percentage_points: "Процентные пункты",
  relative_percent: "Относительный процент",
  fixed: "Фиксированная сумма",
  hybrid: "Процент + фикс",
  override: "Заменить ставку",
  included: "Уже включено",
  processing_volume: "Оборот обработки",
  revenue_share: "Доля от выручки",
  fixed_referral: "Фикс за рекомендацию",
  manual: "Ручное начисление",
  projected: "Рассчитано",
...
export const humanizeCode = (value?: string | null) => {
  if (!value) return "—";
  const text = statusLabels[value] || value.split("_").join(" ");
