      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-5">
        ...
        {tab === "overview" && !workspaceLoading && <>
          ...
          <div className="grid gap-5 xl:grid-cols-3">
            ...
            <OverviewPanel title="Открытые задачи" count={openTasks.length} action="Работать с задачами" onOpen={() => setTab("tasks")}>
              <div className="space-y-3">{openTasks.slice(0, 3).map((task) => <article key={String(task.id)}><div className="flex items-start justify-between gap-3"><strong className="text-sm text-gray-800 dark:text-gray-200">{task.title}</strong><span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-500 dark:bg-white/5">{statusLabels[String(task.priority || "normal")] || String(task.priority || "normal")}</span></div><p className="mt-1 text-xs text-gray-400">{task.due_at ? `Срок: ${dateTime(task.due_at)}` : "Без срока"}</p></article>)}{!openTasks.length && <p className="text-sm text-gray-400">Открытых задач нет.</p>}</div>
            </OverviewPanel>
        ...
        <Section title="Основная информация">
          ...
          {entityType === "casino" ? <>
            ...
            <Field label="Статус PSP"><select className={inputClass} value={String(draft.provider_status || "research")} onChange={(event) => set("provider_status", event.target.value)}>{["research", "qualified", "partner", "paused", "rejected"].map((value)=><option key={value} value={value}>{statusLabels[value] || value}</option>)}</select></Field>
        ...
        <Section title="Контакты и коммуникация">
          ...
          <Field label="Состояние контакта"><select className={inputClass} value={String(draft.contact_status || "not_contacted")} onChange={(event) => set("contact_status", event.target.value)}>{["not_contacted", "researching", "ready", "contacted", "replied", "negotiating", "partner", "rejected", "paused"].map((value)=><option key={value} value={value}>{statusLabels[value] || value}</option>)}</select></Field>
