...
export default function MerchantWorkspace() {
  ...
  async function requestComplianceScreening() {
    ...
    await runAction("compliance-screening", async () => {
      ...
      return { error: result.error };
    }, "Проверка поставлена в очередь. Worker проверяет очередь раз в минуту, по одной заявке за запуск; результат появится после обработки. Постановка в очередь ещё не означает завершение проверки.");
  }
