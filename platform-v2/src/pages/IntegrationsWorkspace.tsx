...
const moduleMeta: Record<string, { label: string; description: string }> = {
  docling: { label: "Docling", description: "Опциональный OCR fallback. Обычные PDF, DOCX и таблицы разбираются встроенными parser-узлами." },
  ...
  mem0: { label: "Mem0", description: "Опциональная внешняя семантическая память. Боевая память AIBot уже хранится в Supabase-профиле BIXOFFPSP." },
  posthog: { label: "PostHog", description: "Сознательно не используется для staff-поведения. Бизнес-аналитика берётся из Supabase, публичные page views — из Vercel." },
};
...
async function loadIntegrationHealth(signal?: AbortSignal) {
  ...
  return { checks: result.checks as Health, checked_at: String(result.checked_at || new Date().toISOString()) } satisfies IntegrationHealthResult;
}
...
function moduleStatus(item: PlatformModule) {
  if (item.optional && !item.enabled && item.substitute_healthy) return { label: "Не требуется", style: "bg-brand-50 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300" };
...
export default function IntegrationsWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    if (healthResult.status === "rejected") {
      ...
      setHealth(healthResult.value.checks);
      setHealthCheckedAt(healthResult.value.checked_at);
    }
  ...
  const status = (item?: ConnectorHealth) => {
    ...
    const label = verified ? "Доступ подтверждён" : item?.configured ? "Ошибка связи" : "Не настроено";
  ...
  return <>
    ...
    <Panel className="mt-5">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
        {(moduleItems.length ? moduleItems : Object.keys(moduleMeta).map((name): PlatformModule=>({ name }))).map((item) => {
          ...
          return <div key={item.name} className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-white/[0.04]">
            ...
            {item.substitute && <p className="mt-3 text-xs leading-5 text-brand-600 dark:text-brand-300">Рабочая замена: {item.substitute}.</p>}
            {(item.error || (item.reason && !item.substitute_healthy)) && <p className="mt-3 text-xs text-error-600 dark:text-error-300">{item.error || (item.reason === "disabled_or_unconfigured" ? "Модуль выключен или не настроен." : item.reason)}</p>}
      ...
      <p className="mt-4 rounded-xl bg-gray-50 px-4 py-3 text-xs leading-5 text-gray-500 dark:bg-white/[0.03] dark:text-gray-400">«Не требуется» означает: внешний сервис не подключён намеренно, а указанная рабочая замена уже покрывает боевой сценарий. Это не зелёная маскировка отсутствующей интеграции.</p>
