...
function RouteEditor({ route, reload, select, execute }: { providerId:string; route:Route; reload:()=>Promise<void>; select:(id:string)=>void; execute:RouteWorkspaceProps["execute"] }) {
  ...
  return <Panel>
    ...
    <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <Field label="Категория бизнеса"><select disabled={!editable} className={fieldClass} value={draft.riskMode} onChange={e=>setDraft({...draft,riskMode:e.target.value})}><option value="low">Low-risk</option><option value="high">High-risk</option><option value="both">Low + High</option></select></Field>
