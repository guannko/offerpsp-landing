...
function ComponentRows({title,kind,rows,setRows,disabled}:{title:string;kind:"fees"|"limits"|"settlements";rows:JsonRow[];setRows:(v:JsonRow[])=>void;disabled:boolean}){const fields=kind==="fees"?["flow","fee_type","base_percent","base_fixed","base_fixed_currency","applies_on"]:kind==="limits"?["flow","currency","minimum_amount","maximum_amount"]:["currency","fee_percent","period","minimum_amount","exchange_rule"];return <div><div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-semibold text-gray-900 dark:text-white">{title}</h3>{!disabled&&<button onClick={()=>setRows([...rows,{}])} className="text-xs font-semibold text-brand-500">+ Добавить</button>}</div><div className="space-y-2">{rows.map((row,index)=><div key={index} className="grid grid-cols-2 gap-2 rounded-xl bg-gray-50 p-3 md:grid-cols-6 dark:bg-white/[0.03]">{fields.map(field=><input key={field} disabled={disabled} className={fieldClass} placeholder={field} value={String(row[field]??"")} onChange={e=>{const next=[...rows];next[index]={...row,[field]:e.target.value||null};setRows(next)}}/>)}{!disabled&&<button onClick={()=>setRows(rows.filter((_,i)=>i!==index))} className="text-xs text-error-600">Удалить</button>}</div>)}</div></div>}
