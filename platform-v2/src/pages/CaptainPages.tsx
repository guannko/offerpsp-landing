...
export function IntegrationsWorkspace() {
  ...
  const hasAIBotData = Boolean(captainsBridge.telegram_log.length || captainsBridge.bot_tasks.length);
  const hasTelegramLog = Boolean(captainsBridge.telegram_log.length);
  const systems = [
    { name: "Supabase", state: health ? (health.supabase && lastUpdatedAt ? "success" : "warning") : "pending", status: health ? (health.supabase && lastUpdatedAt ? "Работает" : "Требует внимания") : "Проверяю…", detail: `данные обновлены ${lastUpdatedAt?.toLocaleTimeString("ru-RU",{hour:"2-digit",minute:"2-digit"})||"—"}` },
    { name: "n8n / AIBot", state: hasAIBotData ? "success" : "neutral", status: hasAIBotData ? "Данные загружены" : "Нет рабочих данных", detail: `${captainsBridge.telegram_log.length} сообщений · ${captainsBridge.bot_tasks.length} задач; это наличие данных, а не прямой health-check n8n` },
    { name: "Email Sender", state: health ? (health.n8n_email_webhook ? "success" : "warning") : "pending", status: health ? (health.n8n_email_webhook ? "Работает" : "Требует внимания") : "Проверяю…", detail: `${captainsBridge.email_drafts.length} записей в журнале` },
    { name: "Telegram / AIBot", state: hasTelegramLog ? "success" : "neutral", status: hasTelegramLog ? "Журнал загружен" : "Журнал пуст", detail: `${captainsBridge.telegram_log.length} сообщений Lead Hunter; пустой журнал сам по себе не означает ошибку Telegram` },
  ];
  return <Frame title="Состояние интеграций" description="Фактическое состояние сервисов и доступных данных."><PageHeading eyebrow="System control" title="Состояние интеграций" description="Отделяем прямую проверку сервиса от простого наличия загруженных данных."/>{healthError&&<ErrorBanner message={healthError}/>}<div className="grid grid-cols-1 gap-4 lg:grid-cols-2">{systems.map((system)=><Panel key={system.name}><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-gray-900 dark:text-white">{system.name}</h2><p className="mt-2 text-sm text-gray-500">{system.detail}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${system.state === "success" ? "bg-success-50 text-success-700 dark:bg-success-500/10 dark:text-success-300" : system.state === "warning" ? "bg-warning-50 text-warning-700 dark:bg-warning-500/10 dark:text-warning-300" : "bg-gray-100 text-gray-600 dark:bg-white/5 dark:text-gray-300"}`}>{system.status}</span></div></Panel>)}</div></Frame>;
}
