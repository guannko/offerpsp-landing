...
export default function OperationsWorkspace() {
  ...
  return <>
    ...
    {mode === "calendar" && <Panel className="mt-5"><div className="offerpsp-calendar"><FullCalendar plugins={[dayGridPlugin, listPlugin, interactionPlugin]} initialView="dayGridMonth" locale="ru" height="auto" firstDay={1} headerToolbar={{ left:"prev,next today", center:"title", right:"dayGridMonth,listMonth" }} buttonText={{ today:"Сегодня", month:"Месяц", list:"Список" }} dateClick={(info)=>editTask(undefined, info.dateStr)} eventClick={(info)=>{const task=operationalTasks.find((item)=>String(item.id)===info.event.id);if(task)editTask(task);}} events={operationalTasks.filter((task)=>task.due_at).map((task)=>({ id:String(task.id), title:task.title || "Задача", start:task.due_at || undefined, color:task.priority==="urgent"?"#f04438":task.priority==="high"?"#fb6514":"#465fff" }))}/></div><p className="mt-4 text-xs text-gray-400">Нажмите на день, чтобы создать задачу со сроком, или на событие, чтобы открыть редактор.</p></Panel>}

    {mode === "aibot" && <Panel className="mt-5"><div className="mb-4"><h2 className="text-lg font-semibold text-gray-900 dark:text-white">Очередь автоматизаций AIBot</h2><p className="mt-1 text-sm text-gray-500">Состояние можно анализировать, но менять нужно через n8n, а не вручную в таблице.</p></div><div className="divide-y divide-gray-100 dark:divide-gray-800">{operationalAibotTasks.map((task)=><div key={task.id} className="grid gap-3 py-4 md:grid-cols-[minmax(0,1fr)_130px_160px]"><div><strong className="text-sm text-gray-900 dark:text-white">{task.task_type || "Миссия AIBot"}</strong><span className="mt-1 block text-xs text-gray-400">{task.payload ? JSON.stringify(task.payload).slice(0, 180) : "без payload"}</span></div><StatusPill status={task.status}/><span className="text-xs text-gray-400 md:text-right">{task.scheduled_for ? new Date(task.scheduled_for).toLocaleString("ru-RU") : "Без расписания"}</span></div>)}{!operationalAibotTasks.length&&<EmptyState title="Миссий нет" description="Автоматические задания AIBot появятся здесь."/>}</div></Panel>}
