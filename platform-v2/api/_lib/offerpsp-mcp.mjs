...
export async function executeOfferPspTool(name, args, { request, context, callId }) {
  ...
  if (name === "ask_offerpsp_agent") {
    ...
    const merchantInstruction = input.entity_type === "merchant" && UUID.test(clamp(input.entity_id, 80))
      ? `CURRENT MERCHANT: ${clamp(input.entity_id, 80)}. Before answering any matching, shortlist or pricing question, call Operating Desk with {"action":"get_matching","lead_id":"${clamp(input.entity_id, 80)}"}. This result is the canonical merchant-specific matching snapshot. search_offers is only published supply and must not be used as proof that calculated client pricing is unavailable. A lead status such as shortlist_ready means candidates exist; claim that a shortlist was saved only when saved_shortlist_item_count is greater than zero. Validate every narrative claim against the exact matches before answering: when match_count is greater than zero, do not claim that matching data is unavailable; when client_pricing contains payin values, do not claim that PayIn pricing is missing. In a client-facing draft, hide provider identity and internal route codes but preserve the exact flows, methods and calculated client pricing. Present client_pricing only as final client rates; never mention internal margin, markup, base rates, provider count or how the client price was calculated. Never expose provider identity in a client-facing draft before a controlled introduction.\n\n`
      : "";
