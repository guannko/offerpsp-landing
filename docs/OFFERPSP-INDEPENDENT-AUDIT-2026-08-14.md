...
**VERIFIED production.** Исправления выпущены из локального коммита `f70161f` в deployment `dpl_G6oxujtGHv8wGG9SDL89qQq6dx3J`. Vercel подтвердил `target: production`, `status: Ready`; основной alias — `https://ops-7q4m2x9k8v3n.vercel.app`.

Production E2E в существующей staff-сессии подтвердил:

- Communications сразу показывает доступный loading status, затем загружает 4 активные цепочки и исходные 2 непрочитанных;
- `Пометить прочитанным` уменьшило счётчик `2 → 1`, кнопка стала `Пометить непрочитанным`;
- обратное действие восстановило счётчик `1 → 2`, кнопку и исходное состояние тестовой E2E-цепочки;
- `Обзор воронки` показывает новое название и пояснение о смене статуса в карточке мерча;
- Integrations показывает loading status, затем фактические данные;
- browser console на проверенных страницах не содержит warning/error.

Runtime logs нового deployment не содержат неуспешных запросов в проверенном сценарии. Один `GET /api/module-health` завершился `200`, но Vercel классифицировал `DeprecationWarning [DEP0169]` от транзитивной Node-зависимости как error-level log. Это не сломало health-check и не является ложным success, но остаётся не блокирующим техническим долгом обновления зависимости под Node 24.

Публикация ветки в GitHub не подтверждена: локальный `gh` не авторизован. Production Vercel и production Supabase работают на проверенном состоянии; удалённая ветка пока отстаёт от локального коммита на один commit.
