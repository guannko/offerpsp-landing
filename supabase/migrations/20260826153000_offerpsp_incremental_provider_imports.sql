create or replace function public.import_offerpsp_rate_card_unguarded(
...
declare
  ...
  v_incremental boolean := lower(coalesce(p_parser_metadata ->> 'submitted_by_provider', 'false')) = 'true'
    or lower(coalesce(p_parser_metadata ->> 'entrypoint', '')) = 'provider_portal';
...
begin
  ...
  -- Provider portal submissions are incremental. A new file or Telegram-style
  -- message can add one GEO without replacing every other route from this PSP.
  -- Supersession is therefore decided per route family after each new route is
  -- normalized, never for the provider's whole previous batch.
  ...
  loop
    ...
    -- A new unpublished version replaces only an unpublished route with the
    -- same normalized family. Published and paused candidates remain live and
    -- continue through the existing explicit staff replacement review.
    if v_incremental then
      update private.offerpsp_offer_routes previous
      set status = 'archived',
          updated_at = now()
      where previous.provider_id = v_provider.id
        and previous.id <> v_route.id
        and previous.batch_id <> v_batch.id
        and previous.status in ('draft', 'review')
        and previous.route_family_key = v_route.route_family_key;
    end if;
...
comment on function public.import_offerpsp_rate_card_unguarded(
  text, text, text, text, date, text, jsonb, jsonb
) is 'Imports provider-portal submissions incrementally while preserving full-snapshot replacement for staff ingestion; unrelated provider routes remain active.';
